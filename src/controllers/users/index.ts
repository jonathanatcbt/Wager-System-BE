...
export class UserController {
    ...
    public getUserDetailsByUserName= async (req:Request,res:Response)=>{
        try{
            const {userName}=req.params
            if (!userName){
                return res.status(400).json({ status: false, message: 'Missing username' });
            }
            let user= await this.userServiceManager.dynamicUserSearchWithAvatar("username",userName)
            res.status(200).json({
                status: true,
                user,
